...
public class DatosIniciales {
    ...
    public static void inicializarSiEsNecesario(ServicioUsuarios servicioUsuarios,
        ...
        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST001", "U001", "Mi primera publicación en la red!",
                        new Date(ahora - 5_000_000), 120, 3, 150));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST002", "U002", "Compartiendo conocimiento sobre programación",
                        new Date(ahora - 8_000_000), 250, 4, 300));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST003", "U003", "Foto del día 📸",
                        new Date(ahora - 2_000_000), 80, 2, 100));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST004", "U004", "Reflexión del fin de semana",
                        new Date(ahora - 10_000_000), 350, 5, 400));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST005", "U005", "Nuevo artículo en mi blog",
                        new Date(ahora - 1_000_000), 45, 1, 60));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST006", "U006", "Tutorial de algoritmos",
                        new Date(ahora - 6_000_000), 180, 3, 220));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST007", "U007", "Evento próximo en la universidad",
                        new Date(ahora - 3_000_000), 95, 2, 120));

        servicioPublicaciones.agregarPublicacion(
                new PublicacionModelo("POST008", "U008", "Logro desbloqueado! 🎯",
        ...
        System.out.println(
                "   - " + servicioPublicaciones.obtenerTodasLasPublicaciones().size() + " publicaciones creadas");
